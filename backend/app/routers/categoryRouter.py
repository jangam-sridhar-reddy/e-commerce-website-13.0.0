...
def create_main_category(category: mainCategoriesSchema.CreateMainCategory, currentUser:UserModel = Depends(getCurrentUser()),db:Session = Depends(get_db)) -> mainCategoryServices.MainCategoryModel:
...
def get_main_categories(currentUser:UserModel = Depends(getCurrentUser()),db:Session = Depends(get_db)) -> List[mainCategoriesModels.MainCategoryModel]:
...
def get_main_category_by_id(categoryId:int,currentUser:UserModel = Depends(getCurrentUser()), db:Session = Depends(get_db)) -> mainCategoriesModels.MainCategoryModel |  Dict[str, str]:
...
def update_main_category(categoryId: int, category: mainCategoriesSchema.CreateMainCategory, currentUser:UserModel = Depends(getCurrentUser()), db:Session = Depends(get_db)) ->  mainCategoriesModels.MainCategoryModel:
    return mainCategoryServices.updateCategory(db=db, id=categoryId, category=category)
    
    
@router.delete('/main/{categoryId}')
def delete_main_category(categoryId:int, currentUser:UserModel = Depends(getCurrentUser()), db:Session =  Depends(get_db)) -> Dict[str, str]:
