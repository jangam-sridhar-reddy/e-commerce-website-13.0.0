...
def create_sub(sub: SubCategoriesCreateSchema, currentUser:UserModel = Depends(getCurrentUser()), db:Session=Depends(get_db)) -> SubCategoriesModel:
    return createSubCategory(sub=sub, db=db)

@router.get('/sub/{ID}', response_model= SubCategoriesSchema)
def get_sub_category_by_id(ID:int, currentUser:UserModel = Depends(getCurrentUser()),) -> SubCategoriesModel: 
    subCategory: SubCategoriesModel = getSubCategoryById(id=ID)
    return  subCategory


@router.get('/sub', response_model=List[SubCategoriesSchema])
def get_sub_categories(skip:int= 0, limit:int = 10, currentUser:UserModel = Depends(getCurrentUser()),) -> List[SubCategoriesModel]:
    return getSubCategories(skip=skip, limit=limit)


@router.put('/sub/{ID}', response_model=SubCategoriesSchema)
def update_sub_category(ID:int, sub: SubCategoriesCreateSchema, currentUser:UserModel = Depends(getCurrentUser()), db:Session= Depends(get_db)) -> SubCategoriesModel:
    return updateSubCategory(id=ID, sub=sub, db=db)

@router.delete('/sub/{ID}')
def delete_sub_category(ID:int, currentUser:UserModel = Depends(getCurrentUser()), db:Session = Depends(get_db)):
